...
public class BoxesDAO implements BoxesDAOLocal {
	...
	public String make(String language, String body, String password,
		...
		/********
		 * Temporary alias because the field does not allow NULL value The alias
		 * is computed starting from the id generated from the database
		 */
	...
	public Revision get(int userId, String alias, String password)
		...
		/************
		 * I need only 1 revision. The revisions are ordered by rev number
		 * (desc)
		 */
	...
	public List<Operation> getOperations(String alias, int from) {
		...
		Query newOperationsQuery = em.createQuery(Queries
				.get("BOXES_OPERATIONS"));
	...
	public Revision getRevision(String alias, Integer revision) {
		...
		Query revisionQuery = em.createQuery(Queries.get("BOXES_REVISIONS"));
		...
		if (rev.size() < 1)
			throw new RuntimeException("Revision not available");
		return rev.get(0);
	}

	private int getMaxRevision(String alias) {
		Query maxRevisionQuery = em.createQuery(Queries.get("BOXES_LASTREVISION"));
